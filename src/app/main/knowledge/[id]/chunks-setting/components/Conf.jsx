...
import { previewCommon, previewParent, saveAndHandleSearch } from '@/api/knowledge'
import List from 'rc-virtual-list'
import ReactMarkdown from 'react-markdown'
import { checkPermission } from '@/utils/utils'
import remarkBreaks from 'remark-breaks'
const ChunkSetting = forwardRef((props, ref) => {
  ...
  const [baseConfChunkSize, setBaseConfChunkSize] = useState(0)
  ...
  useEffect(() => {
    // setBaseConf(prev => ({ ...prev, chunkSize: 555, chunkIdentifier: '',chunkOverlap: 22 }))
    // setBaseConf(prev => ({ ...prev, chunkSize: props.knowledgeBase.chunkMaxLength, chunkIdentifier: props.knowledgeBase.chunkIdentifier ? props.knowledgeBase.chunkIdentifier:baseConf.chunkIdentifier,chunkOverlap: props.knowledgeBase.chunkOverlapLength }))
  }, [])
  ...
  const setParams = data => {
    ...
    // console.log(props.documentList, 'llllll')
    setKnowledgeBase(data)

    setSearchCurrent(data.searchStrategy || 'VECTOR')
    setCurrent(data.chunkingStrategy === 'PARENT_CHILD' ? 2 : 1)

    getAgentModelListEvent(data)
  }
  ...
  const getAgentModelListEvent = kb => {
    ...
    getAgentModelList(data).then(res => {
      ...
      const chunkIdentifiers = data.chunkIdentifier
        ? data.chunkIdentifier.replace(/\n/g, '\\n')
        : baseConf.chunkIdentifier
      // console.log(data.chunkMaxLength, 'chunkMaxLength')

      setBaseConf(prev => ({
        ...prev,
        chunkSize: kb.chunkMaxLength,
        chunkIdentifier: chunkIdentifiers,
        chunkOverlap: kb.chunkOverlapLength,
        enable: kb.enableQaChunk,
        modelId: kb.qaModelId,
      }))

      const paragraphChunkIdentifiers = data.paragraphChunkIdentifier
        ? data.paragraphChunkIdentifier.replace(/\n/g, '\\n')
        : parentConf.chunkIdentifier

      setParentConf(prev => ({
        ...prev,
        chunkSize: kb.paragraphChunkMaxLength,
        chunkIdentifier: paragraphChunkIdentifiers,
      }))

      const childchunkIdentifiers = data.chunkIdentifier
        ? data.chunkIdentifier.replace(/\n/g, '\\n')
        : childChunkSetting.chunkIdentifier
      setChildChunkSetting(prev => ({
        ...prev,
        chunkSize: kb.chunkMaxLength,
        chunkIdentifier: childchunkIdentifiers,
      }))
  ...
  const previewCommonEvent = id => {
    // console.log(documentId, id, 'ddddd')
  ...
  const saveEvent = () => {
    ...
    // console.log(documentId, 'documentIddocumentIddocumentIddocumentId')
