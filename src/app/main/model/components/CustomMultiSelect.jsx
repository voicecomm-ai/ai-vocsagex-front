...
import { modelTagSave } from "@/api/model";
...
const CustomMultiSelect = ({
  ...
  // 已选中的标签对象
  const selectedTags = modelTagList.filter((tag) => selectedIds.includes(tag.id));
  const createNewTag = async () => {
    if (inputValue.trim() === "") {
      return message.warning("标签名不能为空");
    }

    await modelTagSave({ categoryId: tagList.id, name: inputValue });
  ...
  return (
    <div ref={selectRef} className={styles.selectContainer} style={{ width: screenWidth }}>
      {/* 已选标签展示区域 */}
        ...
        <div className={styles.selectTagContainer}>
          ...
          {selectedTags.length > 0 ? (
            <>
              ...
              {selectedTags.length > 3 && <span className={styles.tagEllipsis}>...</span>}
            </>
          ) : (
      ...
      {open && (
        <div className={styles.dropdown}>
          <div className={styles.inputContainer}>
            <Input
              placeholder={!readOnly ? "搜索或创建标签" : "搜索标签"}
              className={styles.searchInput}
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              onClick={(e) => e.stopPropagation()}
              suffix={<img src='/model/search.png' className={styles.searchIcon} />}
            />
            <img
