...
import agentStyles from "./index.module.css";
import { checkApplicationStatus } from "@/api/workflow";
import { NodeSourceHandle, NodeTargetHandle } from "../../node-handle";
const { Paragraph,Text } = Typography;
const AgentNode = ({ id, data, selected, type }) => {
  const { checkApplicationExistEvent } = useTemplate();
  ...
  return (
    <div
      ...
      className={`${styles["custom_node"]} ${agentStyles["agent_node"]} ${
    ...
    >
      ...
      >
        <div className={styles["custom_node_header"]}>
          ...
          <div className={agentStyles["custom_node_header_tag"]}>
          <Text style={{ maxWidth:data.tagList&&data.tagList.length == 1? 48:36,color:'#60687D' }} ellipsis={{ tooltip: tagTitle }}>
            <span className={agentStyles["custom_node_header_tag_text"]}> {tagTitle}</span>
            </Text>
            {data.tagList && data.tagList.length > 1 && (
              <span className={agentStyles["custom_node_header_tag_add"]}>
