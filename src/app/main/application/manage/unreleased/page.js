...
export default function AppManagePage() {
  ...
  const fetchApplications = (
    ...
    const params = {
      ...
      name: keyword,
      ...PAGINATION_CONFIG,
      hasPublish: false,//是否已经发布  false:未发布 true:已发布
  ...
  const editAppInfo = async () => {
    const formValues = await form.getFieldsValue(); // 获取表单最新值
    const editParams = {
      ...currentAppInfo,
      ...formValues,
    };
  ...
  useEffect(() => {
    ...
    // 设置操作权限
    const hasOperationPermission = checkPermission("/main/application/manage/unreleased/operation");
  ...
  return (
    <div className={styles["app-manage-page"]}>
      <div className={styles["page-title"]}>
        <span className={styles["page-title-text"]}>未发布</span>
      ...
      <Modal
        ...
        styles={MODAL_STYLES}
      >
        <Form form={form} layout='vertical' initialValues={currentAppInfo} >
          {/* 应用图标和名称输入区域 */}
          ...
          <Form.Item style={{ marginTop: 32, textAlign: "right" }}>
            ...
            <Button type='primary' onClick={() => editAppInfo()} style={{ width: 112 }}>
              确定
            </Button>
          </Form.Item>
        </Form>
