...
const RelationManagement = forwardRef((props, ontologyRef) => {
  ...
  const [originSubData, setOriginSubData] = useState([]); // 原始关系列表数据
  const [substanceList, setSubstanceList] = useState([]);

  // 关系列表
  const [currentSub, setCurrentSub] = useState(null); // 当前选中关系
  ...
  const [delSubObj, setDelSubObj] = useState(null); // 删除关系数据
      ...
      render: (text, record) => {
        return (
          <div style={{ display: "flex", gap: 8 }}>
            <Button
              type="link"
              onClick={() => openAddEditAttribute("relation", "edit", record)}
              disabled={
  ...
  const delConfirmEvent = () => {
    ...
    if (delType === 1) {
      ...
      if (fullRemove) {
        ...
        const selected = selectedTemp.values();
        const form = Array.from(selected);
        confirmDeleteRelProperty(form);
      }
    }
  };

  const confirmDeleteRelProperty = (data) => {
    dropTagEdgePropertyApi({
      ...currentSub,
      space: currentNamespaceId,
      type: 1,
  ...
  const openSurvivalModel = () => {
    survivalTimeRef.current.showModal("relation");
  };
          ...
          <Col span={4} className={styles["substance-aside"]}>
            <div className={styles["title-wrapper"]}>
              ...
              {isShow && btnPermission && (
                <div
                  ...
                  onClick={() => props.showMainBodyModel("relation")}
                ></div>
              )}
            </div>
            <div className={styles["search-wrapper"]}>
              <Input
                ...
                onPressEnter={() => getRelationshipList()}
                suffix={
                  <SearchOutlined
                    style={{ cursor: "pointer" }}
                    onClick={() => getRelationshipList()}
                  />
                }
              />
            </div>
              ...
              {isShow && (
                <div className={styles["action-button"]}>
                  <Button
                    ...
                    onClick={() => openAddEditAttribute("relation", "add")}
                  >
                    <PlusOutlined />
                    新增属性
                  </Button>
