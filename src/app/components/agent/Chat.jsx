...
import ThinkingTypewriter from "./ThinkingTypewriter"; // 思考类型写入组件
...
// 常量定义
const API_ENDPOINTS = {
  CHAT_TEST: "/voicesagex-console/application-web/chat/test",
};
...
const AgentChat = forwardRef((props, ref) => {
  ...
  const [goNext, setGoNext] = useState(false); //是否滚动到最底部 是否加载下一页
  ...
  const sendEvent = async (value) => {
    ...
    fetchEventSource(chatUrl, {
      ...
      body: JSON.stringify({
        ...
        inputs,
        chatHistory,
        runType: type, //数据类型草稿draft，已发布published，试用experience
      }),
        ...
        <div className={styles["chat_container_header_right"]}>
          ...
          <Tooltip title="显示/隐藏右侧面板">
            <img
              ...
              className={styles["chat_container_header_right_tip"]}
              alt="提示"
            />
          </Tooltip>
        </div>
        ...
        {chatList.length == 0 && (
          <div className={styles["chat_container_empty"]}>
            <img
              src={
                process.env.NEXT_PUBLIC_API_BASE +
                applicationInfo.applicationIconUrl
              }
              alt="icon"
