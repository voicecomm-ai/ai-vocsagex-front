...
const LoopNode = ({ id, data, type }) => {
  ...
  const { handleResizeEvent, calculateChildNodesBounds } = useLoop();
  ...
  const handleNodeSelect = (node,type,nowNodeId) => {
    ...
    let loopStartNode = nodes.find(node => node.type == "loop-start" && node.parentId == id); //获取循环开始节点id
  ...
  const findChildNodes = useCallback((data) => {
    const childNodes = data.filter(node => node.parentId == id &&node.type != "loop-start");
  ...
  useEffect(() => {
    ...
    initializeSize();
    // 当节点数组或ID变化时重新计算
  }, [id, nodeArr, calculateChildNodesBounds, reactFlowInstance]);
  ...
  return (
    <>
      ...
      >
        ...
        {/* Header，阻止拖拽 */}
        <div className={styles.loop_node_header}>
          <img
        ...
        {/* 内容区，允许拖拽子节点 */}
        <div ref={contentRef} className={styles.loop_node_content}>
          <div className={styles.loop_node_content_main}>
            {!isHasChild && (
            <div
              className={styles.loop_node_add_node}
              ref={addRef}
              onClick={(e)=>handleAddNode(e)}
            >
              <div className={styles.loop_node_add_node_line}></div>
              <div className={styles.loop_node_add_node_text}>
                <img
                  className={styles.loop_node_add_node_text_img}
