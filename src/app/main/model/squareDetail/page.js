...
export default function ModelDetailPage() {
  ...
  const modelMap = { 0: "算法模型", 1: "预训练模型" };

  const manufacturerList = [
    { title: "NVIDIA", value: "nvidia" },
    { title: "AMD", value: "amd" },
    { title: "华为昇腾", value: "ascend" },
  ];
  const router = useRouter();
  const backHandle = () => {
    router.push("/main/model/square");
  };
  ...
  useEffect(() => {
    ...
    setUrlParams({
      ...
      isSquare: searchParams.get("isSquare"),
      modelStatus: searchParams.get("modelStatus"),
  ...
  const tabOptions = [
    ...
    // 语音合成展示
    ...(modelDetail.type === 1 && modelDetail.classification === 7
      ? [
          {
            key: 6,
            label: (
      ...
      children: (
        <div className={styles["model_file_option"]}>
          {quantiFile && modelDetail.quantifiedStorageUrl !== "" && (
            <div className={styles["model-option"]}>
              <span className={`${styles["model-file-title"]}`}>已量化导出模型的文件位置</span>
          ...
          {weightFile && modelDetail.weightStorageUrl !== "" && (
            <div className={styles["model-option"]}>
              <span className={`${styles["model-file-title"]} `}>权重文件位置</span>
              <div className={styles["directory-structure"]} style={{ paddingLeft: 12 }}>
          ...
          {modelFile && (
            <div className={styles["model-option-content"]}>
              <p className={styles["directory-tips"]} style={{ marginTop: 0 }}>
                文件可点击查看并下载
              </p>
              <div className={styles["directory-structure"]} style={{ paddingLeft: 12 }}>
                <Spin spinning={fileLoading}>
                  <FileTree data={modelFile} path={modelDetail.path} showUpload='true' />
                </Spin>
              </div>
            </div>
          )}
          {modelDetail.type === 1 && (
            <div className={styles["model-option"]}>
              <span className={`${styles["model-file-title"]} `}>资源配置</span>
