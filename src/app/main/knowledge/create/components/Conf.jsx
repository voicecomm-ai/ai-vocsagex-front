...
import { previewCommon, previewParent, saveAndProcessKnowledge } from '@/api/knowledge'
...
const CreateUpload = forwardRef((props, ref) => {
  ...
  const [current, setCurrent] = useState(1) //1 普通分段 2 高级分段 3向量检索
  ...
  const getAgentModelListEvent = () => {
    ...
    getAgentModelList(data).then(res => {
      ...
      console.log(baseModeArr, 'baseModeArr', data)
      ...
      let multiModalityModelArr = searchModeArr.filter(item => {
        return item.isSupportVisual === searchConf.enableMultimodal
      })
      console.log(multiModalityModelArr, searchConf.enableMultimodal, searchModeArr, 'llll')
      ...
      setSearchConf({
        ...searchConf,
        embeddingModelId: embeddingModeId,
  ...
  const saveEvent = () => {
    ...
    let saveData = {
      ...
      name: arr[0].name ? arr[0].name.slice(0, 50) : '',
      type: 'TRAD',
                ...
                <div className={styles['conf_item_content_rule']}>
                  ...
                  <div className={styles['conf_item_content_column']}>
                    <Checkbox
                      ...
                      style={{ color: baseConf.filterBlank ? 'inherit' : 'rgba(0, 0, 0, 0.45)', width: '257px' }}
                    >
                      替换掉连续的空格、换行符和制表符
                    </Checkbox>
                    <Checkbox
                      ...
                      style={{ color: baseConf.removeUrl ? 'inherit' : 'rgba(0, 0, 0, 0.45)', width: '220px' }}
                    >
                      删除所有 URL 和电子邮件地址
                    </Checkbox>
                  </div>
