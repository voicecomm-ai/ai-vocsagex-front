...
const TestInput = forwardRef((props, ref) => {
  ...
  const runClickEvent = async () => {
    const preserveZero = props.preserveZero; //是否保留0
            ...
            variables.map((item, index) => (
              <div key={index}>
                {item.type === "text-input" && (
                  ...
                  >
                    <Input style={{ background: '#F5F9FC',height: '36px',lineHeight: '36px' }} variant='filled' placeholder='请输入' maxLength={item.max_length} />
                  </Form.Item>
                )}
                {item.type === "paragraph" && (
                  <Form.Item
                ...
                {item.type === "number" && (
                  ...
                  <Form.Item
                    ...
                    rules={[{ required: item.required, message: "请输入" + item.label }]}
                  >
                    <InputNumber style={{ background: '#F5F9FC',height: '36px',lineHeight: '36px',width: "100%" }} variant='filled'  placeholder='请输入' />
                  </Form.Item>
                )}
                {item.type === "select" && (
                  <Form.Item
