...
const TestResult = forwardRef((props, ref) => {
  const {panelVisible,setPanelVisible,pannerNode} = useStore((state) => state);
  const [outputs,setOutputs] = useState(null);
  useImperativeHandle(ref, () => ({

  }));
 
  // 解析结果数据，将 result 转为 label value 的数组
  const parseResult = (result) => {
    ...
    try {
      ...
      // 检查是否有outputs字段
      let outputObj = data.outputs && typeof data.outputs === 'object' ? data.outputs : data;
      ...
      // 不是对象，直接返回
      return [{ label: 'result', value: outputObj }];
    } catch (error) {
  ...
  return (
    <div className={styles['test_result_container']}>
      {!props.result&& !props.isRun &&(
        <div className={styles['test_result_container_empty']}>
          <Empty description="暂无结果" />
        </div>
      )}
      {props.isRun && <div className={styles['test_result_container_loading']}>
        <div className={styles['test_result_container_loading_text']}>
          <span>运行中...</span>
        </div>
      </div>}
      {outputs &&outputs.length > 0 &&( 
       <div className={styles['test_result_container_result']}>
        ...
        <Button className={styles['test_result_container_copy']} type="primary" onClick={() => {
