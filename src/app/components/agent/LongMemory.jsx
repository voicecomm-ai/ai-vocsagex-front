...
const LongMemory = forwardRef((props, ref) => {
  ...
  //获取长期记忆列表
  const getLongMemoryListEvent = (type) => {
    setLoading(true);
    let params = {
      applicationId: id,
      userId: 1,
      type: 'experience',//数据类型 草稿draft，已发布published，试用experience
    };
 ...
 const deleteCallBack = () => {
  ...
  clearAgentLongMemory({ applicationId: id, userId: 1, type: 'experience' })
  .then((res) => {
  ...
  const handleSaveEdit = (e) => {
    ...
    let params = {
      ...
      applicationId: id,
      agentId: agentId,
      dataType: 'experience',//数据类型 草稿draft，已发布published，试用experience
    };
        ...
        <div className={styles.long_memory_content}>
          ...
          <div className={styles.long_memory_list}>
            {loading ? (
              ...
              memoryList.map((item) => (
                <div key={item.id} className={styles.memory_group}>
